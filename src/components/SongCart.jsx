...
const SongCart = ({ song, setIsPlaying }) => {
  ...
  return (
    <div
      ...
      className="px-1 py-1 bg-white/5 backdrop-blur-sm animate-slideup rounded-lg hover:bg-onyx duration-300 cursor-pointer"
    >
      <div className="relative flex flex-col items-center justify-center gap-2 pb-1 pt-4 rounded-lg">
        ...
        />
        <Link className="text-[12px] text-center ">{song.title}</Link>
        <Link className="text-[10px] text-gray-300 text-center">
          {song.subtitle}
        </Link>
      </div>
    </div>
